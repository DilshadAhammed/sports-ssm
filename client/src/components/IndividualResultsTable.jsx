const IndividualResultsTable = ({ results }) => {
  return (
    <div className="relative overflow-x-auto shadow-md sm:rounded-lg mt-6 backdrop-blur-3xl bg-transparent p-4 rounded-lg">
      <h3 className="text-xl font-bold text-center text-white">Individual Results</h3>
        ...
        <tbody>
          {results.map((result) => (
            <tr key={result._id} className="border border-white/10 bg-black/10 backdrop-blur-3xl">
              ...
              <td className="px-6 py-4">{result.first.map((pos) => `${pos.name} (${pos.section.code})`).join(", ") || "-"}</td>
              <td className="px-6 py-4">{result.second.map((pos) => `${pos.name} (${pos.section.code})`).join(", ") || "-"}</td>
              <td className="px-6 py-4">{result.third.map((pos) => `${pos.name} (${pos.section.code})`).join(", ") || "-"}</td>
            </tr>
          ))}
        </tbody>
      </table>
